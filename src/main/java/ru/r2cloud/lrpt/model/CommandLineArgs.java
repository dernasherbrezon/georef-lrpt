...
public class CommandLineArgs {
	...
	@Parameter(names = "--tle-file", description = "File with TLE. Should be from the same date as VCDU", required = true, validateWith = FileValidator.class)
	...
	@Parameter(names = "--irotation-degrees", description = "An angle between satellite camera and nadir")
	private Double irotationDegrees = 2.0;

	@Parameter(names = "--jrotation-degrees", description = "An angle between satellite camera and nadir")
	private Double jrotationDegrees = 0.4;

	@Parameter(names = "--output-dir", description = "Directory where to output the result", validateWith = FileValidator.class)
	private String outputDir = ".";

	@Parameter(names = "--start-time", description = "Time in UTC timezone of the first VCDU file. If not provided, then guess the time from the first vcdu filename. Format: yyyy-MM-dd HH:mm:ss")
	private String startTime;

	@Parameter(names = "--orekit-path", description = "Path where to find orekit urls or where to download them into")
